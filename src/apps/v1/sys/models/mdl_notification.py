...
class NotificationBase(SQLModel):
    ...
    type: NotificationType = Field(
        default=NotificationType.SYSTEM, description="通知类型"
    )
    status: NotificationStatus = Field(
        default=NotificationStatus.UNREAD, description="通知状态"
    )
    recipient_id: int = Field(..., description="接收者ID")
    sender_id: int | str | None = Field(
        default=None, sa_type=sa.String, description="发送者ID"
    )
    read_at: datetime | None = Field(default=None, description="阅读时间")
    meta_data: dict[str, Any] | None = Field(
        default=None, sa_type=sa.JSON, description="元数据"
    )
