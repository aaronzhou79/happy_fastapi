...
from src.apps.v1.sys.models.mdl_opera_log import OperaLogCreate
...
class OperaLogMiddleware(BaseHTTPMiddleware):
    ...
    async def execute_request(
        ...
        try:
            ...
            log.error(f"请求异常: {str(getattr(e, 'data', e))}")
        ...
        return RequestCallNext(
            code=code, msg=msg, status=status, err=err, response=response
        )
    ...
    def desensitization(args: dict) -> dict | None:
        ...
        def _encrypt_value(value: str, cipher_type: OperaLogCipher) -> str:
            """加密单个值"""
            encrypt_map = {
                OperaLogCipher.AES: lambda x: (
                    AESCipher(settings.OPERA_LOG_ENCRYPT_SECRET_KEY).encrypt(x)
                ).hex(),
                OperaLogCipher.MD5: Md5Cipher.encrypt,
                OperaLogCipher.ITSDANGEROUS: lambda x: ItsDCipher(
        ...
        for key in args.keys():
            if key in settings.OPERA_LOG_ENCRYPT_KEY_INCLUDE:
                args[key] = _encrypt_value(
                    args[key], OperaLogCipher(settings.OPERA_LOG_ENCRYPT_TYPE)
                )
        return args
