...
class TreeModel(DatabaseModel, SQLModel):
    ...
    parent_id: int | None = Field(
        default=None, index=True, sa_column_kwargs={"comment": "父节点ID"}
    )
    tree_path: str = Field(
        default="/",
        index=True,
    ...
    level: int = Field(default=1, sa_column_kwargs={"comment": "节点层级"})
    sort_order: int = Field(default=0, sa_column_kwargs={"comment": "排序号"})
    ...
    async def get_siblings(
        self, session: AuditAsyncSession, include_self: bool = False
    ) -> Sequence["TreeModel"]:
        """获取同级节点"""
        stmt = (
    ...
    async def get_children(
        ...
        stmt = (
            select(self.__class__)
            .where(self.__class__.parent_id == self.id)  # type: ignore
            .order_by(asc(self.__class__.sort_order))
        )
        if not include_self:
            stmt = stmt.where(self.__class__.id != self.id)  # type: ignore
    ...
    async def get_ancestors(
        ...
        # 从path中获取所有祖先ID
        ancestor_ids = [int(id_) for id_ in self.tree_path.strip("/").split("/") if id_]
        ...
        stmt = (
            select(self.__class__)
            .where(self.__class__.id.in_(ancestor_ids))  # type: ignore
    ...
    async def create(
        ...
        if isinstance(obj_in, dict):
            ...
            create_data = obj_in.model_dump(exclude_unset=True, exclude=exclude_fields)
