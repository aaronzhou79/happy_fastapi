...
class TreeService(BaseService, TreeCRUD):
    ...
    async def before_delete(
        self, session: AuditAsyncSession, db_obj: TreeModel
    ) -> None:
        """删除节点前钩子"""
    ...
    async def get_tree(
        ...
        return await self.tree_crud.get_tree(
            session, root_id=root_id, max_depth=max_depth
        )
    ...
    async def move_node(
        ...
        return await self.tree_crud.move_node(
            session, node_id=node_id, new_parent_id=new_parent_id
        )
    ...
    async def bulk_move_nodes(
        ...
        return await self.tree_crud.bulk_move_nodes(
            session, node_ids=node_ids, new_parent_id=new_parent_id
        )
    ...
    async def get_siblings(
        ...
        return await self.tree_crud.get_siblings(
            session, node_id=node_id, include_self=include_self
        )
